...
namespace FalconOcr.App.Pages
{
    ...
    {
        private readonly ComboBox _pdfText = Field.Combo("Use the PDF text layer when present (fast, exact)", "Always run OCR (ignore embedded text)");
        ...
        private readonly CheckBox _cls = new CheckBox { Text = "Correct upside-down text lines (orientation classifier)", AutoSize = true };
        private readonly CheckBox _figures = new CheckBox { Text = "Keep pictures and graphics as images", AutoSize = true };
        ...
        public OcrOptionsEditor()
        {
            ...
            _pdfText.Width = 360;
            Row("PDF input:", _pdfText);
            Row("PDF rendering resolution (dpi):", _pdfDpi);
            Row("Detector max. image side (px):", _detMax);
            Row("Text pixel threshold:", _detThr);
            Row("Text box threshold:", _boxThr);
            Row("Box expansion (unclip ratio):", _unclip);
            Row("Minimum line confidence:", _minConf);
            Row("CPU threads (0 = automatic):", _threads);
            Row("", _cls);
            Row("", _figures);
        }
    ...
    internal sealed class AdvancedSettingsDialog : Form
    {
        public AdvancedSettingsDialog(AppSettings settings)
        {
            ...
            var defaults = new Button { Text = "Defaults", Width = 90, Height = 30 };
    ...
    internal static class Prompt
    {
        public static string Show(IWin32Window owner, string title, string caption, string value)
        {
            ...
            })
            {
                ...
                var cancel = new Button { Text = "Cancel", DialogResult = DialogResult.Cancel, Location = new Point(319, 78), Width = 85 };
