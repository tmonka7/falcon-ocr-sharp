...
namespace FalconOcr.App.Pages
{
    ...
    internal sealed class ActivationForm : Form
    {
        ...
        public ActivationForm(LicenseCheck current, bool startup, TrialStatus trial = null)
        {
            ...
            string subtitle = trial == null ? "Enter your license key."
                : trialOpen ? $"You are using the trial version — {trial.DaysLeft} of {trial.TotalDays} days left."
                : "Your trial period has ended. A license key is required to continue.";
            ...
            header.Paint += (s, e) =>
            {
                ...
                TextRenderer.DrawText(e.Graphics, "Activate Falcon OCR", new Font("Segoe UI Semibold", 16f), new Point(70, 12), Color.White);
            ...
            var bannerText = new Label
            {
                ...
                Text = trial == null ? "" : trial.Message + (trialOpen ? "  All features are available during the trial." : "")
            };
            banner.Controls.Add(bannerText);
            banner.Visible = trial != null;
            int y = trial != null ? 154 : 94;

            var step1 = new Label { Text = "1. Send this machine code to your vendor (only needed for computer-bound licenses):", Location = new Point(20, y), AutoSize = true };
            ...
            var copyMachine = new Button { Text = "Copy", Location = new Point(332, y + 25), Width = 80, Height = 30 };
            copyMachine.Click += (s, e) => { Clipboard.SetText(MachineIdentity.Code); _status.ForeColor = Theme.Accent; _status.Text = "Machine code copied to the clipboard."; };

            var step2 = new Label { Text = "2. Enter or paste the license key:", Location = new Point(20, y + 76), AutoSize = true };
            ...
            var loadFile = new Button { Text = "Load license file…", Location = new Point(20, by), Width = 150, Height = 34 };
            bool offerTrial = trialOpen && startup;
            var activate = new Button { Text = "Activate", Location = new Point(offerTrial ? 266 : 388, by), Width = 104, Height = 34, BackColor = Theme.Accent, ForeColor = Color.White, FlatStyle = FlatStyle.Flat, Font = Theme.Bold };
            activate.FlatAppearance.BorderSize = 0;
            var cancel = new Button { Text = startup ? "Exit" : "Cancel", Location = new Point(496, by), Width = 104, Height = 34, DialogResult = DialogResult.Cancel };
            var continueTrial = new Button { Text = "Continue Trial", Location = new Point(378, by), Width = 112, Height = 34, DialogResult = DialogResult.Ignore, Visible = offerTrial };

            loadFile.Click += (s, e) =>
            {
                using (var dlg = new OpenFileDialog { Filter = "License file|*.lic;*.key;*.txt|All files|*.*" })
        ...
        private new void Activate()
        {
            ...
            Result = check;
            MessageBox.Show(this, "Thank you — Falcon OCR is activated.\n\n" + check.Message, "Activated", MessageBoxButtons.OK, MessageBoxIcon.Information);
